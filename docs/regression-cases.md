...
| --- | --- | --- | --- | --- |
| DOR-P0-001 | 主链路 | 在 1280×720 真实浏览器中 submit -> queued -> poll -> delivered -> reply -> completed -> 页面自动更新 -> acknowledge -> close | 提交按钮可真实点击；requestId 始终一致；页面无需用户手工 poll 或刷新即可展示修订；close 后 poll 返回 `review_closed` | `tests/review-lifecycle-regression.test.ts`、`tests/browser/review-lifecycle.spec.ts` |
