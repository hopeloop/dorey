...
提交成功 -> 等待审核 -> foreground poll 收到反馈 -> Agent 回复成功
         -> 页面自动获取 completed 结果 -> acknowledge -> 结束评审
```
...
- 全量本地门禁：`npm test && npm run typecheck && npm run build`
...
| --- | --- | --- | --- | --- |
| DOR-P0-001 | 主链路 | submit -> queued -> poll -> delivered -> reply -> completed -> 页面恢复 -> acknowledge -> close | requestId 始终一致；页面无需用户手工 poll 或刷新即可取回结果；close 后 poll 返回 `review_closed` | `tests/review-lifecycle-regression.test.ts` |
