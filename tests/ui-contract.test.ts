...
describe("review workspace UI contract", () => {
  ...
  it("keeps Codex submit results readable and explains execution visibility", async () => {
    ...
    assert.match(appSource, /处理结果已返回；有修订时可以查看差异并接受/);
    ...
    assert.match(appSource, /\/acknowledge/);
    assert.match(appSource, /unacknowledged: "1"/);
    ...
    assert.match(bootstrapSource, /deliveryMode\?: "foreground" \| "preview"/);
    assert.doesNotMatch(viteConfig, /runCodexDesktopWake/);
    assert.match(viteConfig, /createRevisionPresenceMiddleware/);
    assert.match(viteConfig, /createRevisionReviewMiddleware/);
    assert.match(viteConfig, /DOREY_STATE_ROOT/);
  ...
  it("keeps the real review sidebar compact without removing queue editing", async () => {
    ...
    assert.match(appSource, /全文修订要求（可选）/);
    assert.match(appSource, /这里的内容始终会作为文档修改要求/);
    assert.match(appSource, /aria-label="清空全文修订要求"/);
    assert.match(appSource, /onClick=\{\(\) => setGlobalInstruction\(""\)\}/);
    assert.match(appSource, /aria-label="评论类型"/);
    assert.match(appSource, /Agent 会在原对话回答，不修改原文/);
    assert.match(appSource, /个问题已在原 Agent 对话中回答/);
    assert.doesNotMatch(appSource, /Agent 解释/);
    assert.match(styles, /\.conversation-reply-notice/);
    assert.match(appSource, /添加修订/);
    assert.match(appSource, /添加解释/);
    assert.match(appSource, /提交问题/);
    assert.match(appSource, /提交修订/);
    assert.doesNotMatch(appSource, /澄清|纠错|改写|补充信息|结构调整/);
    assert.doesNotMatch(styles, /\.category-pill/);
    assert.match(styles, /\.comment-kind-control/);
    assert.match(styles, /\.comment-kind-badge/);
